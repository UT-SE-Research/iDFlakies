...
public class RandomDetector extends ExecutingDetector {
    ...
    public RandomDetector(final String type, final File baseDir, final Runner runner,
    ...
    public DetectionRound results() throws Exception {
        lastRandomResult = runList(testShuffler.shuffledOrder(absoluteRound.get(),
            ...
            lastRandomDetectionRound == null || lastRandomResult == null
                || lastRandomDetectionRound.filteredTests().size() != 0));
        // if we want to reverse a run with no failures or errors, then replace the line above with the following:
        // lastRandomResult.results().values().stream().anyMatch(testResult -> testResult.result() == Result.FAILURE
        //     || testResult.result() == Result.ERROR)
