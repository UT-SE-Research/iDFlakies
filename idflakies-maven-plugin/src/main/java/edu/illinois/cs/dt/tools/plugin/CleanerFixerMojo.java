...
import edu.illinois.cs.dt.tools.fixer.FixerResult;
import edu.illinois.cs.dt.tools.fixer.FixStatus;
import edu.illinois.cs.dt.tools.fixer.JavaFile;
import edu.illinois.cs.dt.tools.fixer.JavaMethod;
import edu.illinois.cs.dt.tools.fixer.Patch;
import edu.illinois.cs.dt.tools.fixer.PatchResult;
...
public class CleanerFixerMojo extends AbstractIDFlakiesMojo {
    ...
    public void execute() {
        ...
        logger.runAndLogError(() -> {
            ...
            if (runnerOption.isDefined()) {
                ...
                if (!Files.exists(PathManager.originalOrderPath()) && MinimizerMojo.ORIGINAL_ORDER == null) {
                    Files.write(PathManager.originalOrderPath(), DetectorMojo.getOriginalOrder(this.mavenProject, this.runner.framework()));
                }
    ...
    private boolean applyPatchesAndRun(final List<String> failingOrder,
        ...
        for (Patch patch : patches) {
            ...
            // Try the patch out
            MvnCommands.runMvnInstall(this.mavenProject, false);
            boolean passWithPatches = testOrderPasses(failingOrder);
            patch.restore();        // Regardless, restore patch file(s)
            MvnCommands.runMvnInstall(this.mavenProject, false); // Rebuild again, in preparation for next run
            if (passWithPatches) {
    ...
    private FixStatus checkCleanerRemoval(List<String> failingOrder, JavaMethod cleanerMethod, NodeList<Statement> cleanerStmts) throws Exception {
        try {
            ...
            restore(cleanerMethod.javaFile());
            cleanerMethod = JavaMethod.find(cleanerMethod.methodName(), testSources(), classpath()).get();    // Reload, just in case
            MvnCommands.runMvnInstall(this.mavenProject, true);
        }
    }
    ...
    private Object[] findValidMethodToModify(JavaMethod cleanerMethod, List<String> failingOrder,
        ...
        for (Boolean newTestClass : new boolean[]{true, false}) {
            for (ImmutablePair<JavaMethod, Boolean> tuple :
                ...
                CleanerFixerDeltaDebugger debugger = new CleanerFixerDeltaDebugger(this.mavenProject, this.runner, helperMethod, failingOrder, prepend);
    ...
    private PatchResult applyFix(final List<String> failingOrder,
        ...
        final CleanerFixerDeltaDebugger finalDebugger = new CleanerFixerDeltaDebugger(this.mavenProject, this.runner, finalHelperMethod, failingOrder, finalPrepend);
        ...
        restore(methodToModify.javaFile());
        methodToModify = JavaMethod.find(methodToModify.methodName(), testSources(), classpath()).get();   // Reload, just in case
        CleanerFixerDeltaDebugger debugger = new CleanerFixerDeltaDebugger(this.mavenProject, this.runner, methodToModify, failingOrder, finalPrepend);
    ...
    private NodeList<Statement> debugFurther(NodeList<Statement> stmts, JavaMethod helperMethod,
        ...
        for (int i = 0; i < stmts.size(); i++) {
            ...
            if (stmt instanceof BlockStmt) {
                ...
                debugger = new CleanerFixerBlockDeltaDebugger(this.mavenProject, this.runner, helperMethod, failingOrder, prepend, blockStmt, stmtsToRun);
                ...
                debugger = new CleanerFixerBlockDeltaDebugger(this.mavenProject, this.runner, helperMethod, failingOrder, prepend, tryStmt.getTryBlock(), stmtsToRun);
                NodeList<Statement> minimalBlockStmts = NodeList.nodeList();
                ...
                if (tryStmt.getFinallyBlock().isPresent()) {
                    debugger = new CleanerFixerBlockDeltaDebugger(this.mavenProject, this.runner, helperMethod, failingOrder, prepend, tryStmt.getFinallyBlock().get(), stmtsToRun);
