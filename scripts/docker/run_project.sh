...
timeout 1h /home/$SCRIPT_USERNAME/apache-maven/bin/mvn idflakies:detect ${MVNOPTIONS} -Dtestplugin.className=edu.illinois.cs.dt.tools.utility.ModuleTestTimePlugin -fn -B -e -Ddt.cache.absolute.path=/Scratch/all-output/${modifiedslug}_output |& tee module_test_time.log
...
timeout ${timeout}s /home/$SCRIPT_USERNAME/apache-maven/bin/mvn idflakies:detect ${MVNOPTIONS} ${IDF_OPTIONS} -Ddetector.detector_type=reverse |& tee reverse_original.log
...
timeout ${timeout}s /home/$SCRIPT_USERNAME/apache-maven/bin/mvn idflakies:detect ${MVNOPTIONS} ${IDF_OPTIONS} -Ddetector.detector_type=reverse-class |& tee reverse_class.log
...
timeout ${timeout}s /home/$SCRIPT_USERNAME/apache-maven/bin/mvn idflakies:detect ${MVNOPTIONS} ${IDF_OPTIONS} -Ddetector.detector_type=original |& tee original.log
...
timeout ${timeout}s /home/$SCRIPT_USERNAME/apache-maven/bin/mvn idflakies:detect ${MVNOPTIONS} ${IDF_OPTIONS} |& tee random_class_method.log
...
timeout ${timeout}s /home/$SCRIPT_USERNAME/apache-maven/bin/mvn idflakies:detect ${MVNOPTIONS} ${IDF_OPTIONS} -Ddetector.detector_type=random-class |& tee random_class.log
...
timeout ${timeout}s /home/$SCRIPT_USERNAME/apache-maven/bin/mvn idflakies:detect ${MVNOPTIONS} ${IDF_OPTIONS} -Ddetector.detector_type=smart-shuffle |& tee smart_shuffle.log
